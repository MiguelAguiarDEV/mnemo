...
func Mount(mux *http.ServeMux, store *cloudstore.CloudStore, authSvc *auth.Service, cfg DashboardConfig) {
	...
	// Phase 9: Traces (jarvis-dashboard)
	mux.HandleFunc("GET /dashboard/traces", withCookieAuth(authSvc, h.handleTraces))
	mux.HandleFunc("GET /dashboard/traces/session/{id}", withCookieAuth(authSvc, h.handleTracesSession))
...
// handleTraces renders the recent tool call traces page.
func (h *handlers) handleTraces(w http.ResponseWriter, r *http.Request) {
	userID := getUserIDFromContext(r)
	username := getUsernameFromContext(r)
	isAdmin := h.isAdmin(r)

	var traces []cloudstore.ToolCall
	var total int
	if h.store != nil {
		traces, total, _ = h.store.RecentToolCalls(userID, 50)
	}

	content := TracesPage(traces, total)
	page := Layout("Traces", username, "traces", isAdmin, content)
	page.Render(r.Context(), w)
}

// handleTracesSession renders the detail view for a single session's traces.
func (h *handlers) handleTracesSession(w http.ResponseWriter, r *http.Request) {
	userID := getUserIDFromContext(r)
	username := getUsernameFromContext(r)
	isAdmin := h.isAdmin(r)
	sessionID := r.PathValue("id")
	...
	var total int
	if h.store != nil {
		result, err := h.store.SessionToolCalls(userID, sessionID, 200, 0)
		if err == nil {
	...
	content := TracesSessionPage(sessionID, traces, total)
	page := Layout("Session Traces", username, "traces", isAdmin, content)
	page.Render(r.Context(), w)
}
