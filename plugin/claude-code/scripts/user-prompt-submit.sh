...
# Claude Code to load all mnemo memory tools (which are deferred by default).
#
...
if [ ! -f "$STATE_FILE" ]; then
  ...
  TOOL_MSG="CRITICAL FIRST ACTION — Execute this ToolSearch NOW before responding to the user:"$'\n'"select:mcp__plugin_mnemo_mnemo__mem_save,mcp__plugin_mnemo_mnemo__mem_search,mcp__plugin_mnemo_mnemo__mem_context,mcp__plugin_mnemo_mnemo__mem_session_summary,mcp__plugin_mnemo_mnemo__mem_get_observation,mcp__plugin_mnemo_mnemo__mem_save_prompt"$'\n\n'"After loading tools, call mem_context to check for prior session history before responding."
...
SESSION_START=""
if [ -n "$SESSION_ID" ]; then
  SESSION_START=$(curl -sf "${MNEMO_URL}/sessions/${SESSION_ID}" --max-time 0.2 2>/dev/null \
...
LAST_SAVE_JSON=$(curl -sf \
  "${MNEMO_URL}/observations?project=${ENCODED_PROJECT}&limit=1&sort=created_at:desc" \
