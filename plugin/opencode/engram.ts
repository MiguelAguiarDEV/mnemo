...
const CLOUD_URL = process.env.ENGRAM_CLOUD_URL // e.g. http://100.71.66.54:8080
const CLOUD_API_KEY = process.env.ENGRAM_CLOUD_API_KEY
...
export const Engram: Plugin = async (ctx) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Cloud trace: POST tool call to engram cloud (fire-and-forget)
      if (CLOUD_URL && CLOUD_API_KEY && sessionId) {
        const outputStr = typeof output === "string" ? output : JSON.stringify(output)
        const isEngram = ENGRAM_TOOLS.has(input.tool.toLowerCase())
